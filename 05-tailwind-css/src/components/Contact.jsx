...
const Contact = () => {
        ...
        return (
          <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
            <h3 className="text-xl font-semibold text-white mb-6 flex items-center gap-2">
              ...
              Personal Information <span className="text-sm text-neutral-500 font-normal">(Required)</span>
            </h3>
          ...
          <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div>
              ...
              <select name="duration" value={formData.duration} onChange={handleChange} className={`${inputClasses} appearance-none cursor-pointer`}>
                <option value="" className="bg-neutral-900">Select duration</option>
                {durations.map(d => <option key={d} value={d} className="bg-neutral-900">{d}</option>)}
              </select>
            </div>
          </div>
        );
            ...
            <div className="grid sm:grid-cols-2 gap-5">
              <div>
                ...
                <select name="database" value={formData.database} onChange={handleChange} className={`${inputClasses} appearance-none cursor-pointer`}>
                  <option value="" className="bg-neutral-900">Select database</option>
                  {databases.map(db => <option key={db} value={db} className="bg-neutral-900">{db}</option>)}
                </select>
              </div>
          ...
          <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="p-4 rounded-xl bg-white/5 border border-white/10">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <p className="text-neutral-400">Name: <span className="text-white">{formData.fullName || '-'}</span></p>
                <p className="text-neutral-400">Type: <span className="text-white">{formData.projectType || '-'}</span></p>
                <p className="text-neutral-400">Duration: <span className="text-white">{formData.duration || '-'}</span></p>
                <p className="text-neutral-400">Database: <span className="text-white">{formData.database || '-'}</span></p>
              </div>
            </div>
          </div>
        );
  ...
  return (
    <section id="contact" className="relative w-full py-20 bg-neutral-950 overflow-hidden">
      {/* Background */}
      <div className="absolute inset-0">
        <div className="absolute top-0 left-1/4 w-96 h-96 bg-red-600/10 rounded-full blur-[150px]" />
        <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-purple-600/10 rounded-full blur-[150px]" />
      </div>

      <div className="relative z-10 w-full max-w-[1600px] mx-auto px-4 sm:px-8 lg:px-16 xl:px-24">
        {/* Header */}
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-red-600/10 border border-red-500/20 mb-6">
            ...
            <span className="text-sm text-red-400 font-medium">Start Your Project</span>
          </div>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="relative p-6 sm:p-8 rounded-3xl bg-white/5 border border-white/10">
            {isSubmitted ? (
                  ...
                  <div className="flex gap-3 mt-8">
                    {step > 1 && (
                      <button
                        type="button"
                        onClick={prevStep}
                        className="px-6 py-3 rounded-xl border border-white/20 text-white hover:bg-white/5 transition-all flex items-center gap-2"
                      >
                    ...
                    {step < 4 ? (
                      ...
                      <button
                        ...
                        className="flex-1 px-6 py-3 rounded-xl bg-gradient-to-r from-red-600 to-red-700 text-white font-medium hover:shadow-lg hover:shadow-red-600/25 transition-all disabled:opacity-70 flex items-center justify-center gap-2"
                      >
                        {isSubmitting ? (
                          <>
